...
export default function DependencyHelper() {
  ...
  const handleSubmit = async () => {
    ...
    // Placeholder API (replace with Dependency Helper SDK)
    setTimeout(() => {
      setMessages((prev) => [
        ...prev,
        { role: 'assistant', content: `Analyzed: ${input} (stub response)` },
      ]);
      setLoading(false);
    }, 1000);
  };
  ...
  return (
    <>
      <Head>
        <title>DPS - Dependency Helper</title>
      </Head>
      <Box minH="100vh" bg={bgColor}>
        <AdminNavbar
          secondary={false}
          brandText="DPS - Dependency Helper"
          logoText="DPS"
          setApiKey={() => {}}
        />
        <Flex
