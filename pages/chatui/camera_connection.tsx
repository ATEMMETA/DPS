...
const CameraConnection = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        toast({ title: 'Connection Failed', description: result.error, status: 'error', duration: 3000 });
      }
    } catch (error) {
  ...
  return (
    <Box p={8} maxW="500px" mx="auto">
      <Text fontSize="2xl" mb={4}>Connect Your Camera</Text>
      <form onSubmit={handleSubmit}>
        <VStack spacing={4}>
          ...
          <FormControl>
            ...
            <Input value={wifiSsid} onChange={(e) => setWifiSsid(e.target.value)} placeholder="Your Wi-Fi Name" />
          </FormControl>
          <FormControl>
          ...
          <Button type="submit" colorScheme="blue">Connect Camera</Button>
        </VStack>
      </form>
    </Box>
  );
